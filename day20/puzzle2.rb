...
CONSTANTS = {
  # file: 'sample.txt', threshold: 50, answer: 285, time: '61 ms',
  file: 'input.txt', threshold: 100, answer: 987695, time: '2,561 ms',
}
...
savings = map.collect.with_index do |row, x|
  row.collect.with_index do |cell, y|
    ...
    x_spread
      ...
      .select { |cheat_x, cheat_y| ((x - cheat_x).abs + (y - cheat_y).abs) <= 20 }
